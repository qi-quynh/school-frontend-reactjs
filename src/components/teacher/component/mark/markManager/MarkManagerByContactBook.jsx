...
const MarkManagerByContactBook = () => {
  let { id } = useParams();
  if (id == null) id = -1;
  console.log(id);
  ...
  useEffect(() => {
    const loadGradeEdit = async () => {
      if (id === -1) {
        ...
        dispatch(
          getAllMark(`/mark-student/homeroom-teacher?contactBookId=${id}`)
        );
      }
    };
    loadGradeEdit();
  ...
  const onClickSignIn = (event) => {
    ...
    dispatch(getMarkByStudentId(`/mark-student/teacher?`));
  };
...
export default MarkManagerByContactBook;

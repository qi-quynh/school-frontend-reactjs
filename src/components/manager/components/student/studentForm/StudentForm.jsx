...
const StudentForm = () => {
  ...
  useEffect(() => {
    const loadStudentEdit = async () => {
      ...
      if (id !== -1) {
        await dispatch(getStudentById(`/student/manager/${id}`));
  ...
  const handleSubmit = (values) => {
    ...
    params.classId =
      values.classId !== "" ? parseInt(values.classId) : listClass[0].classId;
    params.name = values.name;
    params.phone = values.phone !== "" ? values.phone : "0";
    params.address = values.address;
    params.email = values.email;
    params.status = true;
    params.schoolId = selectedOption;

    console.log(params);
    console.log(values);
    if (id === -1) {
      params.id = values.id;
      dispatch(addStudent("/student/manager", params, history));
      console.log("add");
    } else {
      params.id = id;
      console.log(params);
      dispatch(updateStudent("/student/manager", params, history));
      console.log("update");
    }
  };

  const handleSchoolChange = (value, selectOptionSetter) => {
    selectOptionSetter(value);

    dispatch(getAllClass(`/class/manager?schoolId=${value}`));
  };
            ...
            <div className="row">
              <div className="col-md-6">
                ...
                <select
                  ...
                  onChange={(val) =>
                    handleSchoolChange(val.target.value, setSelectedOption)
                  }
                >
                  {listSchool.map((item) => (
              ...
              <SelectField label="Lớp" name="classId" type="number">
                {listClass == null
                  ? null
                  : listClass.map((item) => (
                      <option key={item.id} value={item.id}>
